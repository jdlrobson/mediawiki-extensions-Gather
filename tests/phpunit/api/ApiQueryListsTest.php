...
class ApiQueryListsTest extends GatherTestCase {
	...
	public function testMinitems() {
		$list1 = $this->createList( 'gatherUser', [ 'P1', 'P2' ] );
		$list2 = $this->createList( 'gatherUser', [ 'P1', 'P2', 'P3' ] );
		$list3 = $this->createList( 'gatherUser', [ 'P1', 'P2', 'P3', 'P4', 'P5' ] );
		...
		$this->assertArrayEquals( [ $list2, $list3 ], $minitems3 );
	}
	...
	public function testVisibility() {
		...
		$list5 = $this->createList( 'gatherUser', [ 'P1' ] );
		...
		$list6 = $this->createList( 'gatherUser', [ 'P1' ] );
		$this->setListPermissionOverride( $list6, 'hidelist' );
		$this->doApiRequestWithWatchToken( [
			'action' => 'editlist',
			'id' => $list6,
			'label' => 'new test label',
		], null, false, static::$users['gatherUser']->getUser() );

		$normalList = $this->getListIdsFromResults( $this->doApiRequest( [
			'action' => 'query',
			'list' => 'lists',
			'lstmode' => 'allpublic',
		] ) );
		$this->assertArrayEquals( [ $list1, $list4, $list5 ], $normalList );

		$hiddenlList = $this->getListIdsFromResults( $this->doApiRequest( [
			'action' => 'query',
			'list' => 'lists',
			'lstmode' => 'allhidden',
		], null, false, static::$users['sysop']->getUser() ) );
		$this->assertArrayEquals( [ $list2, $list3, $list6 ], $hiddenlList );

		$reviewList = $this->getListIdsFromResults( $this->doApiRequest( [
			'action' => 'query',
			'list' => 'lists',
			'lstmode' => 'review',
		], null, false, static::$users['sysop']->getUser() ) );
		$this->assertArrayEquals( [ $list3, $list4, $list6 ], $reviewList );
	}
